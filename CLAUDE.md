...
- [x] **Implementation-status showcase** — single registry `api/feature_registry.py` (14 shipped / 5 planned, bilingual, bump `VERSION` to re-show popup) feeds: home auto-popup on eCommerce workspace (`ecommerce_workspace_beta.js`, once per version via `osi_showcase_seen_version` localStorage, feature Open/Guide links + User Guide/API Docs buttons + recent-changes tail from OSI Changelog) + "Status 实施进度" section at top of `/osi-docs`. Endpoint `api/status.get_implementation_status`. `docs_check` 8/8 PASS.
